...
class LoginAuthentificationAuthenticator extends AbstractFormLoginAuthenticator implements PasswordAuthenticatedInterface
{
    ...
    public function getUser($credentials, UserProviderInterface $userProvider)
    {
        ...
        if (!$this->csrfTokenManager->isTokenValid($token)) {
            $this->flashy->error("token  invalide");
            throw new InvalidCsrfTokenException();
        }
        $user = $this->entityManager->getRepository(User::class)->findByEmailOrPseudo($credentials['email']);
        if (!$user) {
            // fail authentication with a custom error
            $this->flashy->error(" email ou pseudo inexistant ");
            throw  new CustomUserMessageAuthenticationException(" email ou pseudo inexistant");
        }
        return $user[0];
    }
